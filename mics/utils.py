...
class Utils:
    ...
    async def check_expires(profile:models.Profile):
        ...
        if not is_pay: 
            raise HTTPException(status_code=402, detail="Your access token is expired. Contact with us https://mind-market.ru/pay to pay for access")
    ...
    async def check_profile_access_token(self, access_token:str, checking_expires:bool=True):
        ...
        if not profile :
            raise HTTPException(status_code=401, detail="Your access token is not exist.")
        if checking_expires:
